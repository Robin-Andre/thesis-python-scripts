...
def write(result, experiment, seed):
    with open(SPECS.EXP_PATH + f"CombineExperiment/{experiment}_{seed}.csv", "w+") as file:
        file.write(result)
...
def main():
    ...
    csv = "\n".join(output)
    print(csv)
    with open(SPECS.EXP_PATH + "CombineExperiment.csv", "w+") as file:
        file.write(csv)
