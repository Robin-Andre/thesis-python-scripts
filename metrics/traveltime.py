...
class TravelTime(Metric):
    ...
    def draw(self, group="tripMode", reference=None, suptitle=None):
        if type(group) is not list:
            ...
            listgroup = group
        temp = metric.reduce(self._data_frame, listgroup, "durationTrip", "count")
        temp2 = None
        if reference is not None:
            temp2 = metric.reduce(reference._data_frame, listgroup, "durationTrip", "count")

        col_seperator="tripMode"
        if "tripMode" not in listgroup:
            col_seperator = None
        return visualization.generic_plot(temp, group, "count", "durationTrip", reference_df=temp2, color_seperator=col_seperator, sharex=False, suptitle=suptitle)
